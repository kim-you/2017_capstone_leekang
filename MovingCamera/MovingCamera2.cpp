...
#define ROTATION 2 //Define how many rotation it is
...
int main(){
	...
	int* min = new int(ROTATION);
	Size _resize = Size(400, 300);
	...
	Mat src, gray, edge, back, diff, back2, bob, bob2, edge2;
	Mat fortesting;
	Mat back_gray;
	...
	while (capture.read(src)){
		...
		if (flag == 0){
			...
			for (int i = 0; i < ROTATION; i++){
				...
				foreground[i].create(_resize,0);
				background[i].create(_resize,0);
			}
			continue;
		}
...
//			printf("QUEUE_AVERAGE = %f\n", average_w);
		...
		if (camera_flag==0 && turn_flag==true){
			...
			vc[number]->apply(frames[number],foreground[number]);
			vc[number]->getBackgroundImage(background[number]);
			medianBlur(background[number], background[number], 3);
			//blur(background[number], background[number], Size(3,3));

			cvtColor(background[number], back_gray, CV_BGR2GRAY);
			Canny(back_gray, back_gray, 50, 100);
			imshow("TEST2", back_gray);
			cout << "MINIMUM : " << min[number] << "    ///   CURRENT : " << countNonZero(back_gray) << endl;
//			cout << "MIN = " << min[number] << " COUNT = " << countNonZero(back_gray) << endl;
			
			if (countNonZero(back_gray) < min[number] && countNonZero(back_gray) > 1000 && flag > 15){
				//cout << "[" << number << "]" << "Updated: " << countNonZero(back_gray) << endl;				
				min[number] = countNonZero(back_gray);
				min_back[number] = background[number].clone();

				//imwrite("TEST.PNG", min_back[number]);
...
*/
		if (!background[number].empty())
		imshow("BACKGROUND", background[number]);
